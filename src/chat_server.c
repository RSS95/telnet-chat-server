...
void acceptNewRegister(int new_fd)
{
    char *buf;
    int len;
    ...
    while(1)
    {
        ...
        {
            perror("Server New Register::: Error Reading from Socket");
        }
        
        if (register->capacity - registry->length < 10)
        {
            increaseRegisterCapacity();
        }
